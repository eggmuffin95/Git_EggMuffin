...
# Script correspondant aux volumes des instances taggées en "monthly"

export PATH=$PATH:/usr/local/bin/:/usr/bin

# On récupère tous les ID des volumes EBS connectés aux instances taggées en backup "monthly"
ec2_volume_ebs_id_list=`aws ec2 describe-instances --region eu-west-1 --filters "Name=tag:backup,Values=monthly" --output text --query 'Reservations[*].Instances[*].BlockDeviceMappings[*].Ebs.VolumeId'`
...
do
	# On récupère l'ID des' instances taggées en backup "monthly"
	ec2_volume_instance_id=`aws ec2 describe-volumes --region eu-west-1 --volume-id ${ec2_volume_ebs_id} --output text --query 'Volumes[*].Attachments[*].InstanceId'`
  	# On récupère le nom des instances taggées en backup "monthly"
  ...
  aws ec2 create-snapshot --region eu-west-1 --output=text --description "Monthly - $ec2_volume_instance_name - $ec2_volume_ebs_id" --volume-id "$ec2_volume_ebs_id" --query SnapshotId

	# Pour chaque Volume, on récupère le nombre de Snapshots et on stocke les résultats unitaires dans une variable
	Snapshot_info=/tmp/Snapshot_monthly_info.txt
