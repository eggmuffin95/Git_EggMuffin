...
sudo apt-get install -y vim nginx mysql-client php5-fpm php5-cli php5-common php5-mysql git php5-intl curl php5-apcu php5-curl
...
sudo sed -i "s/user = www-data/user = admin/" /etc/php5/fpm/pool.d/www.conf
sudo sed -i "s/group = www-data/group = www-data/" /etc/php5/fpm/pool.d/www.conf
...
sudo sed -i "s/;date.timezone =.*/date.timezone = Europe\/Paris/" /etc/php5/fpm/php.ini
